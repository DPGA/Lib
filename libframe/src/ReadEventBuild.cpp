...
int main()
{
  ...
  int NombreDeFragmentATester =  0 ; /* 59796;*/
  ...
  const char * W_FileName = "Test.bin";
  FILE * File = fopen(W_FileName,"w");
  
  
  //name file
	DecodeFrame DecFrame("/home/gate/Documents/DAQ/ServeurUdp/build/Write_File.bin");
  ...
  int PrintTheHeader 	= 1; // 1-> print only "Fragment & Pointer Rawdata & Start Of Frame" 2 -> All
  int PrintChannels 	= 2;
  int PrintCRC_Eof	= 0;
  ...
  do {
    ...
    if(!pRawData) break; // ATTENTION NECESSARY FOR STOP AT THE END OF THE File
    ...
    if(PrintChannels >1 || Index >197){
      cout << "Channel before map " <<endl;
      for (int i=0;i<4;++i){
    ...
    if( pRawData && HdrFrame ){
      ...
      int size_temp = (HeaderFile->NbSamples*2+8);
      
      uint16_t * pRawData_Copy = (uint16_t *) calloc(4,size_temp);
   
//       memcpy (pRawData_Copy, pRawData, sizeof(uint16_t));
      
      
//        *pRawData_Copy = *pRawData;
        memcpy (pRawData_Copy, pRawData,4*size_temp);
...
//       uint16_t * pRawData_Copy = pRawData;
	...
	for(int i =0; i< map_Channel[CptTriggerThor].size();i++ ){
	  ...
	  uint16_t Eof= DecFrame.GetEof(pRawData_Temp);
	  
	  cout << "CRC " << CRC << endl;
	  cout << "Eof " << Eof << endl;
	  
	  // 	  cout << "copy ?? " << pRawData_Temp << endl;
...
// 	  fwrite(pRawData_Temp,(((1024+(2))*4)+2),2,File ); 
	  ...
	  // 	  fwrite(HdrFrame_Temp,sizeof(S_HeaderFrame),1,File ); 
	  // // 	  fwrite(pRawData_Temp,((NbSample_Temp+(2))*NbrChannel),2,File ); 
	  // 	  
	  // 	  fwrite(pRawData_Temp,sizeof(uint16_t),1,File ); 
	  
	  // 	  fwrite(HdrFrame_Temp,sizeof(S_HeaderFrame),1,File ); 
	  // 	  fwrite(pRawData_Temp,((NbSample_Temp*2+4)*NbrChannel)+4,1,File ); 
	  
	  // 	  pRawdata[((GetNbSamples()+(2))*NbrChannel)+1]
	  
	  // 	  HeaderFile->NbSamples*2+4)*4+4
	}
...
// 		map_NecessaryInformation.erase(CptTriggerThor);
  ...
  for(auto& x: map_Channel){
    cout<<" Nb Triggger Thor aka Event " << x.first << " Nbr tot paquet" << map_CompteurPaquetThor[z] << endl;
    //     if(z >10) break;
    z++;
  }
